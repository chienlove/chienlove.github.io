...
/* ===================== Breadcrumb (mũi tên, không vỡ, rút gọn) ===================== */
function Crumb({ href, children, last = false }) {
  const Comp = href ? Link : 'span';
  return (
    <div className="flex items-center min-w-0">
      <Comp
        href={href || '#'}
        className={`relative inline-flex items-center h-10 px-4 whitespace-nowrap min-w-0 max-w-full ${
          last
            ? 'bg-white text-sky-600 border-2 border-sky-500'
            : 'bg-sky-500 text-white hover:bg-sky-600'
        } rounded-lg`}
      >
        <span className="truncate max-w-[48vw] sm:max-w-[40vw] md:max-w-[30vw]">{children}</span>
      </Comp>
    </div>
  );
}
...
function LoginModal({ open, onClose, onLoggedIn }) {
  ...
  const login = async (e) => {
    ...
    try {
      ...
      setErr(er?.message || 'Không đăng nhập được');
    ...
    <CenterModal
      ...
      body={
        <form onSubmit={login} className="space-y-3">
          ...
          <p className="text-[11px] text-gray-500">Sau khi đăng nhập, vui lòng xác minh email nếu được yêu cầu.</p>
        </form>
      }
      actions={
        <>
          <button
            onClick={loginGoogle}
            disabled={busy}
            className={`px-3 py-2 text-sm rounded border ${busy ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50'}`}
            type="button"
          >
            Đăng nhập Google
          </button>
          <button
            onClick={login}
            disabled={busy}
            className={`px-3 py-2 text-sm rounded bg-gray-900 text-white ${busy ? 'opacity-50 cursor-not-allowed' : ''}`}
            type="button"
          >
...
export default function Detail({ serverApp, serverRelated }) {
  ...
  /* ===== Modal "Cần xác minh email" – nút Đăng nhập mở popup Layout nếu có, nếu không thì mở LoginModal nội bộ ===== */
  const openGlobalLoginPopup = useCallback(() => {
    try {
      if (typeof window !== 'undefined') {
        ...
        // nếu Layout không listen -> sẽ không có popup, fallback dưới
      }
    } catch {}
    return false;
  }, []);
    ...
    setModal({
      ...
      actions: (
        <>
          <button
            onClick={() => {
              const opened = openGlobalLoginPopup();
              setModal(s => ({ ...s, open: false }));
              if (!opened) setShowLoginModal(true); // Fallback modal nội bộ
            }}
  ...
  return (
    <Layout fullWidth>
      ...
      {/* Modal xác minh / đăng nhập + Modal Login fallback */}
      <CenterModal
        open={modal.open}
        title={modal.title}
        body={modal.body}
      ...
      <LoginModal
        ...
        onLoggedIn={(u) => setMe(u)}
      />

      {/* ===== Breadcrumb (mũi tên, an toàn trên mobile, không xuống dòng) ===== */}
          ...
          <div className="w-full max-w-screen-2xl">
            <nav aria-label="Breadcrumb" className="px-2 py-3">
              <ol className="flex items-center gap-2 whitespace-nowrap overflow-hidden">
                ...
                <li className="min-w-0">
                  {app?.category?.slug ? (
                    <Crumb href={`/category/${app.category.slug}`}>
          ...
          <div className="relative w-full max-w-screen-2xl px-2 sm:px-4 md:px-6 pb-8 bg-white rounded-none">
            <div className="w-full pb-6" style={{ backgroundImage: `linear-gradient(to bottom, ${dominantColor}, #f0f2f5)` }}>
              ...
              <div className="pt-10 text-center px-4">
                ...
                {/* Nút hành động – không xuống dòng, không cuộn ngang */}
