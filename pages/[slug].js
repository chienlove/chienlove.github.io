...
// ✅ Dynamic import an toàn (tối ưu tốc độ, tránh lỗi SSR)
...
// ───────────────── ErrorBoundary (để phần con lỗi không vỡ trang) ─────────────────
import React from 'react';
class ErrorBoundary extends React.Component {
  constructor(props){ super(props); this.state = { hasError:false }; }
  static getDerivedStateFromError(){ return { hasError:true }; }
  componentDidCatch(){ /* có thể log về server nếu cần */ }
  render(){
    if (this.state.hasError) {
      return (
        <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800">
          Không tải được phần này. Vui lòng thử lại sau ít phút.
        </div>
      );
    }
...
export default function Detail({ serverApp, serverRelated }) {
  ...
  const [bgTop] = useState('#eaf2ff'); // dùng màu an toàn, bỏ thư viện tính màu để tránh lỗi
  ...
  // remark-gfm chỉ nạp khi client cần (giảm bundle, tránh lỗi lặt vặt)
  ...
  // ✅ Gating tải IPA đúng yêu cầu (6)
  const handleDownloadIpa = async (e) => {
    e.preventDefault();
    if (!app?.id || !isInstallable) return;

    // Chưa đăng nhập hoặc chưa xác minh → bật modal giữa màn hình
    ...
    <Layout fullWidth>
      ...
      <div className="bg-gray-100 min-h-screen pb-12">
        ...
        <div className="max-w-screen-2xl mx-auto px-2 sm:px-4 md:px-6 mt-6 space-y-6">
          ...
          {/* Mô tả (7: dynamic markdown + ít rủi ro crash) */}
          ...
          {/* Bình luận (7: lazy + ErrorBoundary chống vỡ trang) */}
          <div className="bg-white rounded-xl p-4 shadow">
            <ErrorBoundary>
