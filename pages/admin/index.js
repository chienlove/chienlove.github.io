...
export default function Admin() {
  ...
  // Hàm getFileSize (giữ nguyên)
async function getFileSize(url) {
  try {
    const response = await fetch(url, { method: 'HEAD' });
    if (!response.ok) throw new Error('Không thể lấy thông tin file');
    
    const contentLength = response.headers.get('content-length');
    if (!contentLength) throw new Error('Không tìm thấy kích thước file');
...
useEffect(() => {
  ...
  const timer = setTimeout(async () => {
    try {
      // Chỉ chạy nếu có trường size và link download thay đổi
      const hasSizeField = currentFields.includes('size');
      if (!hasSizeField || !form.download_link) return;
      
      let ipaUrl = form.download_link;
      
      // Xử lý plist nếu là link itms-services
      if (ipaUrl.includes('itms-services://')) {
        const plistUrl = decodeURIComponent(ipaUrl.split('url=')[1]);
        if (!plistUrl) return;
        
        const plistResponse = await fetch(plistUrl);
        if (!plistResponse.ok) throw new Error('Không thể tải plist file');
        
        const plistText = await plistResponse.text();
        const parser = new DOMParser();
        const xmlDoc = parser.parseFromString(plistText, "text/xml");
        ipaUrl = xmlDoc.querySelector("dict > array > dict > string")?.textContent;
        
        if (!ipaUrl) throw new Error('Không tìm thấy URL IPA trong plist');
      }
      
      // Bỏ qua nếu không phải URL IPA
      if (!ipaUrl.endsWith('.ipa')) return;
      
      const fileSize = await getFileSize(ipaUrl);
      
      if (isMounted && fileSize) {
        setForm(prev => ({
          ...prev,
          'size': fileSize
        }));
      ...
      console.error('Lỗi khi lấy kích thước IPA:', error);
      if (isMounted) {
        setErrorMessage('Không thể tự động lấy dung lượng, vui lòng nhập thủ công');
      }
    }
  }, 1000);
...
}, [form.download_link, currentFields]); 
