...
// LUÔN set fromUserId trong cả update lẫn set để thỏa Firestore Rules (write yêu cầu fromUserId == request.auth.uid hoặc toUserId == request.auth.uid).
...
export default function Comments({ postId, postTitle = '' }) {
  ...
  // Chỉ KHAI BÁO 1 LẦN (tránh duplicate state gây lỗi build)
  ...
  /* ------- Modal helpers (login/verify/confirm) ------- */
  ...
  /* ------- Delete comment (kèm xác nhận) ------- */
  const onDelete = async (c) => {
    if (!me) return;
    openConfirm('Bạn có chắc chắn muốn xoá bình luận này?', async () => {
