...
export default function AppCard({ app }) {
  return (
    <Link href={`/${app.slug}`}>
      <a className="block bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
        {app.banner_url ? (
          <img 
            src={app.banner_url} 
            alt={app.name}
            ...
            {app.testflight_url && (
              <span 
                onClick={(e) => {
                  ...
                  window.open(app.testflight_url, '_blank');
                }}
                className="cursor-pointer bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition"
              >
                TestFlight
              </span>
            )}
